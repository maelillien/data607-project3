...
Our general approach consists of 1) scraping job listings from LinkedIn and Indeed, 2) applying a whitelist of data science-relevant words, paired or non-paired, to the scraped text, 3) storing the resulting sets in a SQL database, and 4) performing an exploratory analysis. For scraping, we use R's rvest package, a counterpart to Python's Beautiful Soup. Our whitelist is informed by a review of several online journalistic sources and includes tools, skills, and knowledge bases like "Python", "Web Scraping", and "Machine Learning", among many others. We use a XXXX database for storage. And our analysis relies on R's tidyverse collection of packages.
...
A function that extracts words describing data science knowledge and skills from LinkedIn search results
```{r}
...
A function that extracts words describing data science knowledge and skills from Indeed search results
```{r}
...
# Web Scraping
In this stage, we apply the site-specific functions to extract, aggregate, filter, then store the words in data frames.
...
```{r bars, eval=TRUE}
##LinkedIn
ggplot(data = linkedin[1:10,], aes(x=reorder(words, -occurrences), y=occurrences)) +
  geom_bar(stat="identity") +
  labs(title="Top 10 Data Science Skills per LinkedIn", x="Skills", y="Occurrences (#)") +
  theme(axis.text.x=element_text(angle=45, hjust=1))
##Indeed
ggplot(data = indeed[1:10,], aes(x=reorder(words, -occurrences), y=occurrences)) +
  geom_bar(stat="identity") +
  labs(title="Top 10 Data Science Skills per Indeed", x="Skills", y="Occurrences (#)") +
  theme(axis.text.x=element_text(angle=45, hjust=1))
```

##Points
```{r points, eval=TRUE}
merge <- merge(indeed, linkedin, by="words")
ggplot(data = merge, aes(x = occurrences.x, y = occurrences.y)) + 
  ...
  geom_label_repel(aes(label = ifelse(occurrences.x>300,as.character(words),'')),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
